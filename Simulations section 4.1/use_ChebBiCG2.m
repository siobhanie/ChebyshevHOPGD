...
x=chebfun('x');
f1=0; 
g = @(x) 2*pi^2 + (cos(mu1hat) + mu1hat^4 + sin(x) + x); 
g2 = chebfun(g,[-b0 b0]); 
...
[n,b,comp_size,c,ctilde,last2,tau_list,A0,A1] = helm_setup2(ak2,N,b0,tau,g); 
...
for i=1:maxit
    ...
    vec=apply_M(ak,ak2,n,A0,A1,a1,b0,applyPreCon(dx),N,Pd);
    alpha=rho/((dz')*vec);

    %update the seed system 
    rx=rxold-alpha*(vec);
    rz=rzold-conj(alpha)*(applyPreConT(apply_M(ak,ak2,n,A0,A1,a1,b0,dz,N,Pd')));
    ...
    for k=1:length(mu2s)
        ...
        [res,xreturn] = postprocess(xhat(:,k),k,b,applyPreCon,mu2s,A0,A1,g,mu(k)); 
        res_mat(i+1,k)=res; 
        ...
        if k==length(mu2s)
            ...
            if res<=tol
                ...
                for l=length(mu2s)-1:-1:1 
                    [l,mu2s(l)]
                    [res,xreturn] = postprocess(xhat(:,l),l,b,applyPreCon,mu2s,A0,A1,g,mu(l)); 
                    res_mat(i+1,l)=res; 
...
function [res,xreturn] = postprocess(xhat,k,b,applyPreCon,mu2s,A0,A1,g,mu)
    A=A0+g(mu2s(k))*A1; 
    n=length(A0); 
