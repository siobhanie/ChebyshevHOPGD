...
mu2hat = (a1+b1)/2; 
x=chebfun('x');
f = @(x) 2*pi^2 + (cos(x) + x^4 + sin(mu2hat) + mu2hat); 
...
[n,b,comp_size,c,ctilde,last2,tau_list,A0,A1] = helm_setup1(ak,N,b0,tau,f); 
...
for i=1:maxit
    ...
    vec=apply_M(ak,ak2,n,A0,A1,a0,b0,applyPreCon(dx),N,Pd);
    alpha=rho/((dz')*vec);

    %update the seed system 
    rx=rxold-alpha*(vec);
    rz=rzold-conj(alpha)*(applyPreConT(apply_M(ak,ak2,n,A0,A1,a0,b0,dz,N,Pd')));
    ...
    for k=1:length(mu2s)
        ...
        [res,xreturn] = postprocess(xhat(:,k),k,b,applyPreCon,mu2s,A0,A1,f,mu(k)); 
        res_mat(i+1,k)=res; 
        ...
        if k==length(mu2s)
            ...
            if res<=tol
                ...
                for l=length(mu2s)-1:-1:1 
                    [res,xreturn] = postprocess(xhat(:,l),l,b,applyPreCon,mu2s,A0,A1,f,mu(l)); 
                    [l,mu2s(l)]
                    res_mat(i+1,l)=res; 
...
function [res,xreturn] = postprocess(xhat,k,b,applyPreCon,mu2s,A0,A1,f,mu)
    A=A0+A1*f(mu2s(k)); 
    n=length(A0); 
