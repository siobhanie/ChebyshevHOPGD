function [n,b,comp_size,c,ctilde,last2,tau_list,A0,A2,A3] = helm_setup2(ak,N,b1,tau,f,g2)
...
for i=1:length(akflip)-1
    if i==1
       	last2=[last2 A2*akflip(i+1)]; %prev last i==2
    elseif i==2 
        last2=[last2 A2*akflip(i+1)]; %prev last i==3
    elseif i==length(akflip)-3 %prev last length - 1
        last2=[last2 A2*akflip(end-2)-A2*akflip(end)];     
    elseif i==length(akflip)-2 %new
        last2=[last2 A2*akflip(end-1) + (2*tau/b1)*(A2*akflip(end))]; 
    elseif i==length(akflip)-1
        last2=[last2 A0+A2*f(tau)+A3*g2];
    else
        last2=[last2 A2*akflip(i+1)];
    end  
end
