...
fa = @(mu) cos(mu) + mu^3;
fb = @(mu) sin(mu) + mu^2; 
%A_of_mu = @(mu) A0 + fa(mu)*A2 + fb(mu)*A3;
...
for i=1:length(mu2)
%for i=1
    solution_mat1 = [solution_mat1 make_sol_mat1(mu2(i),fa,fb,b0,mulist,a0,solution_mat1)];
end

function solution_mat1 = make_sol_mat1(mu2hat,fa,fb,b0,mulist,a0,solution_mat1) 
    ...
    for i=1:length(mulist)
        ...
        [n,b,comp_size,c,ctilde,last2,tau_list,A0,A2,A3] = helm_setup2(ak,N,b0,tau,fa,g1); 
         
        Ptau = A0+A2*fa(tau)+A3*g1; 
        [L,U,P,Q]=lu(Ptau); 
        ...
        for i=1:maxit
            ...
            vec=apply_M(ak,ak2,n,A0,A2,A3,a0,b0,applyPreCon(dx),N,Pd);
            alpha=rho/((dz')*vec);
        
            %update the seed system 
            rx=rxold-alpha*(vec);
            rz=rzold-conj(alpha)*(applyPreConT(apply_M(ak,ak2,n,A0,A2,A3,a0,b0,dz,N,Pd')));
            ...
            for k=1:length(mu2s)
                ...
                [res,xreturn] = postprocess(xhat(:,k),k,b,applyPreCon,mu2s,A0,A2,A3,fa,g1,mu(k)); 
                res_mat(i+1,k)=res; 
                ...
                if k==length(mu2s)
                    ...
                    if res<=tol
                        ...
                        for l=length(mu2s)-1:-1:1 
                            [res,xreturn] = postprocess(xhat(:,l),l,b,applyPreCon,mu2s,A0,A2,A3,fa,g1,mu(l)); 
                            res_mat(i+1,l)=res; 
...
function [res,xreturn] = postprocess(xhat,k,b,applyPreCon,mu2s,A0,A2,A3,fa,g1,mu)
    A=A0+A2*fa(mu2s(k))+A3*g1; 
    n=length(A0); 
