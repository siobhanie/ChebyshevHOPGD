...
f2 = @(mu1) 1 + (sin(mu1)); 
f1 = @(mu2) 10 + cos(mu2) + pi*mu2;

mulist=linspace(a1,b1,N1);  
mu2hat = (a0+b0)/2; %freeze mu1
x=chebfun('x');
g1 = f2(mu2hat);  
f2cheb = chebfun(f1,[-b1 b1]); %interpolate 'f2' on the interval of mu2s
...
[n,b,comp_size,c,ctilde,last2,tau_list] = melina_setup1(ak,N,b1,tau,f1,g1,b,A0,dtA1,dtA2); 
...
for i=1:maxit
    ...
    vec=apply_M(ak,ak2,n,A0,dtA1,dtA2,a1,b1,applyPreCon(dx),N,Pd);
    alpha=rho/((dz')*vec);

    %update the seed system 
    rx=rxold-alpha*(vec);
    rz=rzold-conj(alpha)*(applyPreConT(apply_M(ak,ak2,n,A0,dtA1,dtA2,a1,b1,dz,N,Pd')));
...
plot(solution_mat2(:,1))
hold on
plot(solution_mat2(:,2))
plot(solution_mat2(:,3))
plot(solution_mat2(:,4))
plot(solution_mat2(:,5))
