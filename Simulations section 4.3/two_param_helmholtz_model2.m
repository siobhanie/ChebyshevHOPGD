...
%---Sanity check :) 
X1_2b = Model.F1; X2_2b = Model.F2; X3_2b = Model.F3; X4_2b = Model.F4; a_2b = Model.alpha; 
save('a_2b.mat','a_2b');
save('X1_2b.mat','X1_2b');
save('X2_2b.mat','X2_2b');
save('X3_2b.mat','X3_2b');
save('X4_2b.mat','X4_2b');


% k=length(a_2); rel_err=[]; 
% for i=1:N1
%     model_sol=zeros(n,1); 
%     for j=1:k
%         model_sol = model_sol + a_2(j)*X1(:,j)*X2(j)*X4(i,j)*X3((N1-1)/2+1,j); 
%     end
...
%     model_sol=zeros(n,1); 
%     for j=1:k
%         model_sol = model_sol + a_2(j)*X1(:,j)*X2(j)*X4((N1-1)/2+1,j)*X3(i,j); 
%     end
